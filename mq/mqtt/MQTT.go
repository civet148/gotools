...
func (this *ReactMQTT) OnConnect(Client MQTT.Client) {
	...
	if this.ok && this.closed { //断线重连成功后，重新订阅
		...
		if this.bindingkey != "" { //断线之前已订阅过
			this.Consume(this.bindingkey, this.queuename, this.cb)
		}
	}
	this.ok = true
	this.closed = false
...
*/
func (this *ReactMQTT) Publish(strRoutingKey, strData string) (err error) {

	token := this.conn.Publish(strRoutingKey, 1, false, strData)
	...
	if this.debug {
		log.Info("publish data to [%v] routing key [%v] ok", this.mode.String(), strRoutingKey)
	}
	return
}
...
*/
func (this *ReactMQTT) Consume(strBidingKey, strQueueName string, cb mq.FnConsumeCb) (err error) {

	if this.debug {
		log.Debug("Subscribe start with bindingkey [%v] queue [%v] ", strBidingKey, strQueueName)
	}

	if this.bindingkey != "" {
		this.conn.Unsubscribe(this.bindingkey) //断线重连重置订阅状态
	}

	this.bindingkey = strBidingKey;
	this.queuename = strQueueName;
	this.cb = cb
	if token := this.conn.Subscribe(strBidingKey, 2, nil); token.Wait() && token.Error() != nil {
