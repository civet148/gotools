...
import (
	"fmt"
	// log "halis/common/logging"

	"testing"
)

func TestCoord(t *testing.T) {
	fmt.Println(NewCoord().BdToGcj02(1.23, 3.123))
	fmt.Println(NewCoord().Gcj02toBd(1.23, 3.123))
	fmt.Println(NewCoord().Wgs84ToGcj02(1.23, 3.123))
	fmt.Println(NewCoord().Gcj02ToWgs84(1.23, 3.123))
	fmt.Println(NewCoord().Wgs84ToBd(1.23, 3.123))
	fmt.Println(NewCoord().BdToWgs84(1.23, 3.123))
}
...
// 		//log.Info("WGS lat:%f lng:%f -> GCJ lat:%f lng:%f ", test.wgsLat, test.wgsLng, gcjLat, gcjLng)
// 	}
// }
...
// 		//log.Info("GCJ lat:%f lng:%f -> WGS [lat:%f lng:%f] Offset [%f]", test.gcjLat, test.gcjLng, wgsLat, wgsLng, d)
...
// 		log.Info("GCJ lat:%f lng:%f -> WGS [lat:%f lng:%f] DISTANCE [%f]", test.aLat, test.aLng, test.bLat, test.bLng, d)
